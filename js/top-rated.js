$(document).ready(function () {
  ...
  var topRated = "movie/top_rated";
    ...
    function getRandomTopRated() {


      $.ajax({
      url: baseUrl + topRated + keyPrefix + key, // random movie ID
      // statusCode: {
      //     404: function() {
      //       // never blank - 404
      //       getRandom();
      ...
      console.log(pageLast);
          ...
          $.ajax({
            url: baseUrl + topRated + keyPrefix + key + '&page=' + pageNumber, // random movie ID
            // statusCode: {
            //     404: function() {
            //       // never blank - 404
            //       getRandom();
            ...
            var topRatedMovieID = data.results[arrayRandom].id

                    $.ajax({
                    url: baseUrl + 'movie/' + topRatedMovieID + keyPrefix + key, // random movie ID
                    // statusCode: {
                    //     404: function() {
                    //       // never blank - 404
                    //       getRandom();
...
// movie credits (cast)
    $.ajax({
      url: baseUrl + 'movie/' + topRatedMovieID + '/credits' + keyPrefix + key, // random movie ID
    }).done(function(data) {
      console.log('________________');
    ...
    // movie trailer
    $.ajax({
      url: baseUrl + 'movie/' + topRatedMovieID + '/videos' + keyPrefix + key, // random movie ID
