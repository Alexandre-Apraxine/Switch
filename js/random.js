$(document).ready(function () {
  ...
  function getRandom() {
    // random number to make multiple call on the same id
    var random = ~~(Math.random() * 2000); // ~~ = Math.round()
    ...
    $.ajax({
      url: baseUrl + 'movie/' + random + keyPrefix + key, // random movie ID
      statusCode: {
          404: function() {
            // never blank - 404
            getRandom();
      ...
      // $('.getMovieIdSeen').attr('value',movieID);
    ...
    // movie credits (cast)
    $.ajax({
      url: baseUrl + 'movie/' + random + '/credits' + keyPrefix + key, // random movie ID
    }).done(function(data) {
      console.log('________________');
    ...
    // movie trailer
    $.ajax({
      url: baseUrl + 'movie/' + random + '/videos' + keyPrefix + key, // random movie ID
      ...
      //     trailer.push(data.results[i].key);
      //   }
      // }
