$(document).ready(function () {
  ...
  var trending = "movie/popular";

  
    function getRandomTrending() {


      $.ajax({
      url: baseUrl + trending + keyPrefix + key, // random movie ID
      // statusCode: {
      //     404: function() {
      //       // never blank - 404
      //       getRandom();
      ...
      console.log(pageLast);
          ...
          $.ajax({
            url: baseUrl + trending + keyPrefix + key + '&page=' + pageNumber, // random movie ID
            // statusCode: {
            //     404: function() {
            //       // never blank - 404
            //       getRandom();
            ...
            var trendingMovieId = data.results[arrayRandom].id

                    $.ajax({
                    url: baseUrl + 'movie/' + trendingMovieId + keyPrefix + key, // random movie ID
                    // statusCode: {
                    //     404: function() {
                    //       // never blank - 404
                    //       getRandom();
  ...
  // movie credits (cast)
    $.ajax({
      url: baseUrl + 'movie/' + trendingMovieId + '/credits' + keyPrefix + key, // random movie ID
    }).done(function(data) {
      console.log('________________');
    ...
    // movie trailer
    $.ajax({
      url: baseUrl + 'movie/' + trendingMovieId + '/videos' + keyPrefix + key, // random movie ID
